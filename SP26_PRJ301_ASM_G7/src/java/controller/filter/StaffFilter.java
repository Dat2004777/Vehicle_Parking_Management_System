 ...
 * @author dat20
 */
@WebFilter(filterName = "StaffFilter", urlPatterns = {UrlConstants.URL_STAFF + "/*", UrlConstants.URL_STAFF})
public class StaffFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (session != null && session.getAttribute("staff") != null) {
            ...
            if (accountDAO.getAccountRole(employee.getAccountId()).getRole().name().equalsIgnoreCase("staff")) {
                // Cho phép đi tiếp vào Controller
                chain.doFilter(request, response);
                return;
            } else {
                // Đăng nhập rồi nhưng KHÔNG PHẢI ADMIN (vd: là Manager) -> Báo lỗi 403 Không có quyền
                res.sendError(HttpServletResponse.SC_FORBIDDEN, "Bạn không có quyền truy cập khu vực của Staff!");
                return;
            }
        }
