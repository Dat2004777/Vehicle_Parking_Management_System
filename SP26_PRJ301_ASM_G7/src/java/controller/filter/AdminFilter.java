 ...
 * @author dat20
 */
@WebFilter(filterName = "AdminFilter", urlPatterns = {UrlConstants.URL_ADMIN, UrlConstants.URL_ADMIN + "/*"})
public class AdminFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (session != null && session.getAttribute("admin") != null) {
            Employee adminEmp = (Employee) session.getAttribute("admin");

            // Kiểm tra xem Role có đúng là ADMIN không (Lưu ý viết hoa/thường tùy DB của bạn)
            if (accountDAO.getAccountRole(adminEmp.getAccountId()).getRole().name().equalsIgnoreCase("admin")) {
                // Cho phép đi tiếp vào Controller
                chain.doFilter(request, response);
                return;
            } else {
                // Đăng nhập rồi nhưng KHÔNG PHẢI ADMIN (vd: là Manager) -> Báo lỗi 403 Không có quyền
                res.sendError(HttpServletResponse.SC_FORBIDDEN, "Bạn không có quyền truy cập khu vực của Admin!");
                return;
            }
        }
